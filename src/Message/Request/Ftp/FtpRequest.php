...
class FtpRequest extends AbstractRequest
{
	...
	const STATUS_110 = "Restart marker replay . In this case, the text is exact and not left to the particular implementation; it must read: STATUS_MARK yyyy = mmmm = where yyyy is User-process data stream marker, and mmmm server's equivalent marker (note the spaces between markers and \"=\").";
	...
	const STATUS_214 = "Help message.On how to use the server or the meaning of a particular non-standard command. This reply is useful only to the human user.";
	...
	const STATUS_226 = "Closing data connection. Requested file action successful (for example, file transfer or file abort).";
	...
	const STATUS_421 = "Service not available, closing control connection. This may be a reply to any command if the service knows it must shut down.";
	...
	const STATUS_452 = "Requested action not taken. Insufficient storage space in system.File unavailable (e.g., file busy).";
	//5XX - Syntax error, command unrecognized and the requested action did not take place. This may include errors such as command line too long.
